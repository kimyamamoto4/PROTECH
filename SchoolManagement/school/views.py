...
def add_student_view(request):
    ...
    return render(request, 'system/teacher-students.html')
...
def attendance_view_today(request):
    """View for today's attendance records"""
    attendances = Attendance.objects.filter(date=timezone.now().date())
    return render(request, 'system/attendance_view_today.html', {'attendances': attendances})
...
def export_attendance(request):
    ...
    for attendance in Attendance.objects.all():
        writer.writerow([
            ...
            attendance.student.section.school.name,
            attendance.student.section.grade,
            attendance.student.section.name,
            attendance.date,
            attendance.time_in,
            attendance.time_out,
            getattr(attendance, 'status', 'N/A')  # Handle missing status field
        ])
...
def dashboard_view(request):
    """Dashboard view with role-based data"""
    user = request.user
    ...
    if user.is_superuser or user.is_supervisor:  # Supervisor role
        total_teachers = CustomUser.objects.filter(is_teacher=True).count()
        total_admins = CustomUser.objects.filter(is_staff=True).count()
    elif user.is_staff:  # Admin role
        total_teachers = CustomUser.objects.filter(is_teacher=True).count()

    context = {
        'user_role': 'supervisor' if user.is_superuser or user.is_supervisor else 'admin' if user.is_staff else 'teacher',
        'total_teachers': total_teachers,
        'total_students': total_students,
        'total_attendance': total_attendance,
        'total_admins': total_admins,  # Will be None for non-supervisors
    }
    return render(request, 'system/dashboard.html', context)
